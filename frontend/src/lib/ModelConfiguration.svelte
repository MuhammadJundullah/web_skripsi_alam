<script>
  ...
  async function fetchConfidence() {
    try {
      ...
      console.error("Gagal mengambil ambang keyakinan model:", error);
    }
  }
...
<div class="card h-100">
  <div class="card-body">
    ...
    <label for="confidence-slider" class="form-label">Ambang Keyakinan: {confidence.toFixed(2)}</label>
    <input type="range" class="form-range" min="0.1" max="0.9" step="0.05" id="confidence-slider" bind:value={confidence}>
    <div class="form-text">Kotak deteksi dengan nilai keyakinan di bawah ambang ini akan diabaikan.</div>
    {#if status}
