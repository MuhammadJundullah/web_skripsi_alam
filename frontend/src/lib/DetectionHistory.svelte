<script>
  ...
  async function deleteJob(jobId) {
    if (!confirm('Yakin ingin menghapus proses ini beserta file terkait?')) {
      return;
    }
    try {
      ...
      alert(`Gagal menghapus proses: ${e.message}`);
...
<div class="history-container">
  <h2 class="h4 mb-3">Riwayat Deteksi Video</h2>
  {#if isLoading}
    <p>Memuat riwayat...</p>
  {:else if error}
    <p class="error">Gagal memuat riwayat: {error}</p>
  {:else if jobs.length === 0}
    <p>Belum ada proses deteksi video.</p>
  {:else}
